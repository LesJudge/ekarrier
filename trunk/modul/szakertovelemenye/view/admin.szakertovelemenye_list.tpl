...
<form action="" method="POST" name="{$FormName}" id="{$FormName}" class="form form_list" enctype="multipart/form-data">
        <div class="box_top">
                <h2 class="icon pages">Szakértő véleménye</h2>
                <ul class="sorting">
        ...
        <div class="box_content">    
                ...
                <table class="sorting">
                        ...
                        <tr class="data_row">
                                ...
                                <td class="align_left center">{$lista.komprajzNev}</td>
                                <td class="align_left center">{$lista.hozzaadva}</td>
                                <td class="align_left center">{$lista.valaszolo}</td>
                                <td class="align_left center">{$lista.megvalaszolva}</td>
                                ...
                                {if $lista.Aktiv}
                                <button id="statusz_{$for_id}" title="Visszavonás" class="statusz" onclick="return confirmBox('statusz_{$for_id}', 'Biztosan visszavonja az element', 'Visszavonja a(z) <strong>{$lista.elso}</strong> tételt?');" name="{$BtnUnpublish}" value="{$lista.ID}"><span class="ui-icon ui-icon-locked">Visszavonás</span></button>
                                {else}
                                <button id="statusz_{$for_id}" title="Publikálás" class="statusz" onclick="return confirmBox('statusz_{$for_id}', 'Biztosan publikussá teszi az elemet', 'Publikussá teszi a(z) <strong>{$lista.elso}</strong> tételt?');" name="{$BtnPublish}" value="{$lista.ID}"><span class="ui-icon ui-icon-unlocked">Publikálás</span></button>
                                {/if}
                                </td-->
                        </tr>                             
                        {/foreach}
