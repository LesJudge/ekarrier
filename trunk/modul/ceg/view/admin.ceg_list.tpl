...
<form action="" method="POST" name="{$FormName}" id="{$FormName}" class="form form_list" enctype="multipart/form-data">
    ...
    <div class="box_content">    
        ...
        <table class="sorting">
            ...
            <tr class="data_row">
                <th class="checkers">
                    <input type="checkbox" class="select_row" name="{$SelRow.name}[{$lista.ID}]" value="{$lista.ID}"/>
                </th>
                <td class="align_left">
                    <a href="{$APP_LINK}/edit/{$lista.ID}{$LANG_PARAM}" title="Módosítás">{$lista.elso}</a>
                </td>
                <td class="align_left center">{$lista.creator_lastname} {$lista.creator_firstname}</td>
                ...
                <td class="align_left center">
                    {if $lista.modositas_timestamp neq 0}
                    {$lista.modositas_timestamp}
                    {else}
                    <i>Nem lett módosítva!</i>
                    {/if}
                </td>
                <td class="align_left center">{$lista.megtekintve}</td>
                <td class="align_left center">
                {if $lista.Aktiv}
