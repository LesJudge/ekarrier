...
<form action="" method="POST" name="{$FormName}" id="{$FormName}" class="form form_list" enctype="multipart/form-data">
	<div class="box_top">
		...
		<h2 class="icon pages">Jogosultság csoportok</h2>
		<ul class="sorting">
            ...
            {include file='page/admin/view/admin.list_table_header.tpl'}  
                ...
                {foreach from=$Lista key=for_id item=lista}
                    <tr class="data_row">
                        ...
                        <td>
                            {if $lista.Aktiv}
                                <button id="statusz_{$for_id}" title="Visszavonás" class="statusz" onclick="return confirmBox('statusz_{$for_id}', 'Biztosan visszavonja a jogosultság csoportot', 'Visszavonja a(z) <strong>{$lista.elso}</strong> tételt?');" name="{$BtnUnpublish}" value="{$lista.ID}"><span class="ui-icon ui-icon-locked"/>Visszavonás</span></button>
                            {else}
                                <button id="statusz_{$for_id}" title="Publikálás" class="statusz" onclick="return confirmBox('statusz_{$for_id}', 'Biztosan publikussá teszi a jogosultság csoportot', 'Publikussá teszi a(z) <strong>{$lista.elso}</strong> tételt?');" name="{$BtnPublish}" value="{$lista.ID}"><span class="ui-icon ui-icon-unlocked"/>Publikálás</span></button>
                            {/if}
                        </td>
                   </tr>                             
                {/foreach}
