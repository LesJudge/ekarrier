<div class="row col-data-3-cont">
	<div class="col-lg-12">
		<div class="col-data-3">
			<h3>Belépéshez szükséges adatok</h3>
			<div class="form-row">
					<input type="text" id="{$TxtFnev.name}" name="{$TxtFnev.name}" value="{$TxtFnev.activ}" autocomplete="off" placeholder="Felhasználónév *" />
					{if isset($TxtFnev.error)}<div class="ui-state-error">{$TxtFnev.error}</div>{/if} 
			</div><div class="clear"></div>
			
			<div class="form-row">					
					<input type="text" id="{$TxtEmail.name}" name="{$TxtEmail.name}" value="{$TxtEmail.activ}" autocomplete="off" placeholder="E-mail cím *" />
					{if isset($TxtEmail.error)}<div class="ui-state-error">{$TxtEmail.error}</div>{/if} 
			</div><div class="clear"></div>
			
			<div class="form-row">					
					<input type="password" id="{$Password.name}" name="{$Password.name}" value="{$Password.activ}" autocomplete="off" placeholder="Jelszó{if $passwordRequired} *{/if}" />
					{if isset($Password.error)}<div class="ui-state-error">{$Password.error}</div>{/if} 
			</div><div class="clear"></div>
			
			<div class="form-row">					
					<input type="password" id="{$Password2.name}" name="{$Password2.name}" value="{$Password2.activ}" autocomplete="off" placeholder="Jelszó mégegyszer{if $passwordRequired} *{/if}" />
	...
	<div class="col-lg-12">
		<div class="col-data-3">
			...
			<div class="form-row">				
				<input type="text" id="{$TxtVnev.name}" name="{$TxtVnev.name}" value="{$TxtVnev.activ}" autocomplete="off" placeholder="Vezetéknév *"/>
				{if isset($TxtVnev.error)}<div class="ui-state-error">{$TxtVnev.error}</div>{/if} 
			</div><div class="clear"></div>
			
			<div class="form-row">				
				<input type="text" id="{$TxtKnev.name}" name="{$TxtKnev.name}" value="{$TxtKnev.activ}" autocomplete="off" placeholder="Keresztnév *"/>
			...
			<div style="display: none">
					<div class="form-row">
							...
							{html_options id=$SelNem.name name=$SelNem.name options=$SelNem.values selected=$SelNem.activ class='select-type-1'}
							{if isset($SelNem.error)}<div class="ui-state-error">{$SelNem.error}</div>{/if}
					</div><div class="clear"></div>
			
					<div class="form-row">							
							<input type="text" id="{$TxtAnyjaNeve.name}" name="{$TxtAnyjaNeve.name}" value="{$TxtAnyjaNeve.activ}" autocomplete="off" placeholder="Anyja neve"/>
							{if isset($TxtAnyjaNeve.error)}<div class="ui-state-error">{$TxtAnyjaNeve.error}</div>{/if} 
					</div><div class="clear"></div>
			
					<div class="form-row">							
							<input id="{$TxtTelszamVezetekes.name}" name="{$TxtTelszamVezetekes.name}" type="text" value="{$TxtTelszamVezetekes.activ}" autocomplete="off" placeholder="Vezetékes telefonszám"/>
							{if isset($TxtTelszamVezetekes.error)}<div class="ui-state-error">{$TxtTelszamVezetekes.error}</div>{/if}
					</div><div class="clear"></div>
			</div>
			
			<div class="form-row">				
				<input id="{$TxtTelszamMobil1.name}" name="{$TxtTelszamMobil1.name}" type="text" value="{$TxtTelszamMobil1.activ}" autocomplete="off" placeholder="Mobiltelefonszám *" />
				{if isset($TxtTelszamMobil1.error)}<div class="ui-state-error">{$TxtTelszamMobil1.error}</div>{/if}
			</div><div class="clear"></div>
			
			<div style="display: none">
				<div class="form-row">						
					<input id="{$TxtTelszamMobil2.name}" name="{$TxtTelszamMobil2.name}" type="text" value="{$TxtTelszamMobil2.activ}" autocomplete="off" placeholder="Másodlagos mobiltelefonszám *" />
			...
			<div class="form-row">
				...
				{html_options id=$SelVegzettseg.name name=$SelVegzettseg.name options=$SelVegzettseg.values selected=$SelVegzettseg.activ  class='select-type-1'}
...
<div class="form-row checkbox-row">
        <label for="{$ChkHirlevel.name}">
        	<input type="checkbox" id="{$ChkHirlevel.name}" name="{$ChkHirlevel.name}" value="1"  {if $ChkHirlevel.activ}checked="checked"{/if}/>
			Álláshirdetésekre és hírlevélre feliratkozom
		</label>
