...
<form action="" method="POST" name="{$FormName}" id="{$FormName}" class="form form_list" enctype="multipart/form-data">
        <div class="box_top">
                <h2 class="icon pages">Üzenetek</h2>
                <ul class="sorting">
        ...
        <div class="box_content">    
                ...
                <div class="top_filtering">
                        {html_options name=$FilterSeen.name options=$FilterSeen.values selected=$FilterSeen.activ}
                        {html_options name=$FilterTarget.name options=$FilterTarget.values selected=$FilterTarget.activ}    
                ...
                <table class="sorting">
                        ...
                        <tr class="data_row">
                                ...
                                <td class="align_left"><a href="{$APP_LINK}/edit/{$lista.ID}{$LANG_PARAM}" title="Módosítás">{strip_tags($lista.elso)}</a></td>
                                <td class="align_left center">{$lista.szerzo}</td>
                                <td class="align_left center">{$lista.ugyfelnev}</td>
                                <td class="align_left center">{$lista.datum}</td>
                                <td class="align_left center">{$lista.seen}</td>
