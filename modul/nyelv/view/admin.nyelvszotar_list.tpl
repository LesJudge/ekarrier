...
$(function() { {$FormScript}
    $('select[name="{$FilterModul.name}"]').change(function() { $('#{$BtnFilter}').click(); });
    $("#NyelvSzotarListBtnMultiplePublish").hide();
    $("#NyelvSzotarListBtnMultipleUnpublish").hide();
});
</script>
<form action="" method="POST" name="{$FormName}" id="{$FormName}" class="form form_list" enctype="multipart/form-data">
	<div class="box_top">
		<h2 class="icon pages">Nyelv szótár</h2>
	</div>
        ...
        <table class="sorting">
            ...
            {foreach from=$Lista key=for_id item=lista}
                <tr class="data_row">
                    ...
                    <td class="align_left center">{$lista.nyelv_szotar_azon}</td>
               </tr>                             
            {/foreach}
        </table>
