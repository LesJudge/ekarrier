...
class Captcha
{
    ...
    {
        $this->html = '<style>#capcara td{width: 1px;height: 1px;max-width: 1px;max-height: 1px;padding:0px;margin:0px;border:0px;} #capcara table{padding:0px;margin:0px;border:0px;}</style>
						<table id="capcara" cellspacing=0 cellpadding=0>';
        for ($y = 1; $y < $this->height; $y++) {
            $this->html .= '<tr>';
            for ($x = 1; $x < $this->width; $x++) {
                $szinek = imagecolorsforindex($this->im, imagecolorat($this->im, $x, $y));
                $szink  = sprintf("%02x%02x%02x", $szinek['red'], $szinek['green'], $szinek['blue']);
                if ($szink == "ffffff") {
                    $this->html .= '<td style="background-color:transparent;"></td>';
                } else {
                    $this->html .= '<td style="background-color:#' . $szink . ';"></td>';
                }
            }
            $this->html .= '</tr>';
        }
