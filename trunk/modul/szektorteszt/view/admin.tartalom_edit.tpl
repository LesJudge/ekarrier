...
$(function() { {$FormScript}
    ...
    $("#{$TxtKulcsszo.name}").autoSuggest("",{ neverSubmit: true, showResultList:false, asHtmlID:'{$TxtKulcsszo.name}', preFill: '{$TxtKulcsszo.activ}', selectionRemoved: function(elem){ elem.remove();if($("li.as-selection-item").length == 0){ $(".as-values").attr("value", ""); }; } });
    $(".as-selection-item").addClass("ui-widget-content");
    $(".as-selections").addClass("ui-widget-content");
    $('select[name="{$SelKapcsolodo.name}[]"]').multiselect();
});
</script>
...
<form action="" method="post" name="{$FormName}" id="{$FormName}" class="form form_editor " enctype="multipart/form-data">
	<div class="grid_18">      
    	<div class="box_top">
            <h2 class="icon time">Tartalom - [{$edit_mode}]</h2>
        ...
        <div class="box_content padding">
            ...
            <div style="display: none;">
                <input type="hidden" id="{$HidKezdolap.name}" name="{$HidKezdolap.name}" value="{$HidKezdolap.activ}"/>
                <input type="hidden" id="{$HidDefault.name}" name="{$HidDefault.name}" value="{$HidDefault.activ}"/>
            </div> 
            <div class="field">
                ...
                {if $tartalom_kep_nev}
                	<div class="form_row">
                        <label>
                        	<button title="Kép törlése" id="{$BtnDeleteFile}" name="{$BtnDeleteFile}" value="{$tartalom_kep_nev}" onclick="return confirmBox('{$BtnDeleteFile}', '{php}echo LANG_Cnfr_DeletePicTitle;{/php}', '{php}echo LANG_Cnfr_DeletePic;{/php}');">
								<span class="ui-icon ui-icon-circle-close">Kép törlése</span>
							</button>
                        </label>
                        <img class="form-pic" src="{$DOMAIN}pic/{$APP_PATH}/{$tartalom_kep_nev}_75x75"/> 
					</div><div class="clear"></div>
                {/if}
            </div>
            ...
            <div class="field">
                ...
                <div class="form_row">
                    <label>Kiemelt</label>
                    {html_radios name=$ChkKiemelt.name options=$ChkKiemelt.values selected=$ChkKiemelt.activ}
                    {if isset($ChkKiemelt.error)}<p class="error small">{$ChkKiemelt.error}</p>{/if}
				</div>
                <div class="clear"></div>
            </div>
            <div class="field">
	            ...
	            <!--div class="form_row">
	                ...
	                {html_options name=$SelGroup.name options=$SelGroup.values selected=$SelGroup.activ} 
					<span class="ui-icon ui-icon-info tip" title="Milyen csoportba tartozó külső oldali felhasználók láthatják ezt a tartalmat."></span>
					{if isset($SelGroup.error)}<p class="error small">{$SelGroup.error}</p>{/if}
				</div><div class="clear"></div>
	            <div class="form_row">
	                <label for="{$SelKapcsolodo.name}">Kapcsolódó tartalmak</label>
    ...
    {if isset($tartalom_create_date)}
    	<div class="grid_6"> 
        	<div class="box_top">
                <h2 class="icon time">Információ</h2>
            </div>
            <div class="box_content padding">
                <div class="form_row"> <strong>Megtekintve:</strong> {$tartalom_megtekintve} 
					<input class="submit" style="margin-left: 10px; padding: 1px;" type="submit" name="{$BtnDeleteMegtekintes}" value="Törlés"/>
                </div><div class="clear"></div>
                <div class="form_row"> <strong>Javítások száma:</strong> {$tartalom_javitas_szama} </div><div class="clear"></div>
                <div class="form_row"> <strong>Létrehozva:</strong> {$tartalom_create_date} </div><div class="clear"></div>
                <div class="form_row"> <strong>Létrehozó:</strong> {$tartalom_letrehozo} </div><div class="clear"></div>
                <div class="form_row"> <strong>Utoljára módosítva:</strong> {$tartalom_modositas_datum} </div><div class="clear"></div>
                <div class="form_row"> <strong>Utolsó módosító:</strong> {$tartalom_modosito} </div><div class="clear"></div>
            </div>
      </div>
    {/if}
</form>