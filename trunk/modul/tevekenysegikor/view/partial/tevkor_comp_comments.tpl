...
<div onClick="$('#compEditorCont').toggle();">Írj hozzá!</div>
<div id="compEditorCont" style="display:none">
<form action="" method="post" name="{$FormName}" id="{$FormName}">
    <div class="form-row">
				<label for="compComment">Tartalom <span class="require">*</span></label>
				<textarea id="compComment" name="compComment" class="tinymce">{$content}</textarea>
		</div><div class="clear"></div>
		
		<div class="form-row">
				<label>&nbsp;</label>
				<button class="submit btn" name="{$BtnAddCompComment}" id="{$BtnAddCompComment}" type="submit">Beküld</button>
            </div><div class="clear"></div>
</form>
</div>
