...
// Controller
require 'modul/user/library/controller/BaseAdminUserListController.php';
// Model
require 'modul/user/library/model/BaseAdminUserListModel.php';
...
/**
 * @property User_UgyfelList_Model $_model Model.
 * @property Smarty $_view Smarty View.
 */
class UserUgyfel_Admin_Controller extends BaseAdminUserListController
{
    /**
     * Controller neve.
     * @var string 
    ...
    /**
     * A model, amit betölt majd a controller.
     * @var string 
     */
    protected $model = '_UgyfelList';
    /**
     * A nézet, amit betölt majd a controller.
     * @var string 
     */
    protected $view = 'admin.user_ugyfel_list.tpl';
    /**
     * Flash kezelés
     * @var \Flash
     */
    ...
    {
        $exportConfig = require 'modul/user/config/admin.clientIO.config.php';
        ...
        $this->_view->assign('opIntEqual', User_UgyfelList_Model::OP_INT_EQUAL);
        $this->_view->assign('opIntGreaterThan', User_UgyfelList_Model::OP_INT_GREATER_THAN);
        $this->_view->assign('opIntGreaterThanOrEqual', User_UgyfelList_Model::OP_INT_GREATER_THAN_OR_EQUAL);
        $this->_view->assign('opIntLessThan', User_UgyfelList_Model::OP_INT_LESS_THAN);
        $this->_view->assign('opIntLessThanOrEqual', User_UgyfelList_Model::OP_INT_LESS_THAN_OR_EQUAL);
        $this->_view->assign('opLikeAnywhere', User_UgyfelList_Model::OP_LIKE_ANYWHERE);
        $this->_view->assign('opLikeEqual', User_UgyfelList_Model::OP_LIKE_EQUAL);
        $this->_view->assign('opLikePost', User_UgyfelList_Model::OP_LIKE_POST);
        $this->_view->assign('opLikePre', User_UgyfelList_Model::OP_LIKE_PRE);
        $this->_view->assign('opSqlBetween', User_UgyfelList_Model::OP_SQL_BETWEEN);
        $this->_view->assign('client', new \Client);
        $this->_view->assign('clientData', new \ClientData);
    ...
    protected function setDynamicFilters()
    {
        ...
        $binds = array(
            'Firstname' => array(
                'field' => 'user.user_vnev',
                'method' => 'Like'
            ),
            'Lastname' => array(
                'field' => 'user.user_knev',
                'method' => 'Like'
            ),
            'Nickname' => array(
            ...
            'Birthname' => array(
                'field' => 'uu.szuletesi_nev',
                'method' => 'Like'
            ),
            'Birthdate' => array(
                'field' => 'uu.szuletesi_ido',
                'method' => 'Date'
            ),
            'Birthplace' => array(
                'field' => 'uu.szuletesi_hely',
                'method' => 'Like'
            ),
            'MikorReg' => array(
            ...
            'VegzettsegIskola' => array(
                'field' => 'uav.user_attr_vegzettseg_iskola',
                'method' => 'Like'
            ),
            'VegzettsegKezdes' => array(
                'field' => 'uav.user_attr_vegzettseg_kezdet',
                'method' => 'Date'
            ),
            'VegzettsegVegzes' => array(
                'field' => 'uav.user_attr_vegzettseg_veg',
                'method' => 'Date'
            ),
            'VegzettsegSzak' => array(
                'field' => 'uav.user_attr_vegzettseg_szak',
                'method' => 'Like'
            )
        );
        ...
        $this->setDynamicFilterViaClosure('NyelvtudasNyelv', function($filter, $model) use ($eventsExecuted, $name) {
            if ($eventsExecuted == 0) {
                ...
                if (!empty($langs)) {
                    ...
                    $in = rtrim($str, ',');
                    $model->_join .= ' INNER JOIN user_attr_nyelvtudas un ON user.user_id IN (SELECT un.user_id 
                        FROM user_attr_nyelvtudas un
                        WHERE (un.nyelvtudas_nyelv_id, un.nyelvtudas_szint_id) IN (' . $in . ') 
                        GROUP BY un.user_id' . ($all ? 
            ...
            if ($eventsExecuted == 0) {
                try {
                    ...
                    if (is_array($cks) && is_array($likes) && ($ccks == $clikes) && $ccks > 0) {
                        $join = ' INNER JOIN user_attr_szgep_ismeret uasi ON user.user_id IN (SELECT user_id 
                            FROM user_attr_szgep_ismeret WHERE ';
                        $i = 0;
                        $str = '';
                        foreach ($cks as $ck) {
        ...
        $this->setDynamicFilterViaClosure('ProgramInformacio', function($filter, $model) use ($eventsExecuted) {
            if ($eventsExecuted == 0) {
                $model->_join .= ' INNER JOIN user_attr_program_informacio uapi ON user.user_id = uapi.user_id ';
        ...
        $this->setDynamicFilterViaClosure('Munkarend', function($filter, $model) use ($eventsExecuted) {
            if ($eventsExecuted == 0) {
                $model->_join .= ' INNER JOIN user_attr_munkarend uam ON user.user_id = uam.user_id ';
                $model->addDynamicFilterCondition(
    ...
    public function onClick_Export()
    {
        ...
        require 'modul/user/library/startup/admin.clientIOStartup.php'; // ClientIO
        require 'modul/user/library/ClientExport.php'; // Ügyfél export.
        ...
        if (is_array($attributes) && !empty($attributes)) {
            try {
                ...
                $this->_view->assign('FormError', 'RIMÓCZZIIIZIZIII');
            }
        } else {
